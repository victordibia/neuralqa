...
def download_data(data_url, source_name):
    ...
    zip_file_path = os.path.join([data_dir, source_name + ".zip"])
    ...
    data_file = os.path.join(extract_dir, os.listdir(
        extract_dir)[0], data_dir, "data.jsonl.xz")
    shutil.copyfile(data_file, os.path.join(data_dir, "newmexico.json.xz"))
